...
def train_model(model, env, team, num_epochs=1000):
    ...
    for epoch in range(num_epochs):
        ...
        while not done:
            ...
            action = np.argmax(model.predict(state.reshape(1, 10, 10)))  # Adjusted to the correct input shape
            ...
            epoch_loss += loss
            model.fit(state.reshape(1, 10, 10), np.array([action]), epochs=1, verbose=0)  # Adjusted input shape
...
if __name__ == "__main__":
    input_shape = (10, 10)
    env = CaptureTheFlagEnv(grid_size=input_shape)
